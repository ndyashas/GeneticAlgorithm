...
def fitnessFun(agentObj, target, spread):
    return((spread)-(meanSquareError(agentObj.fuNNy(target, len(target)), target)))

def meanSquareError(op, target):
    error = 0
...
if(__name__ == "__main__"):
    ...
    if(1):
        ...
        while(epoch < NUMBER_OF_EPOCH):
            ...
            error = (meanSquareError(target, sess.getBestAgent().fuNNy(target, len(target)))/(SPREAD))*100
